...
// api/_holidayEntriesImpl.ts
var holidayEntriesImpl_exports = {};
__export(holidayEntriesImpl_exports, {
  default: () => handler
});
module.exports = __toCommonJS(holidayEntriesImpl_exports);
...
async function handler(req, res) {
  ...
  if (method === "POST") {
    ...
    const holidayDate = body.holiday_date?.trim();
    const label = typeof body.label === "string" ? body.label.trim() || void 0 : void 0;
    if (!calendarId || !holidayDate) {
      res.status(400).json({ error: "calendar_id and holiday_date required" });
      return;
    }
    const { data: cal, error: calErr } = await client.from("holiday_calendars").select("id, market_id, operating_model_id, segment_id").eq("id", calendarId).single();
    if (calErr || !cal) {
      res.status(404).json({ error: "calendar not found" });
      return;
    }
    const mkt = await lookupMarketMeta(cal.market_id);
    if (!mkt || !scopeAllowsMarketEdit(
      auth.scope,
      cal.market_id,
      mkt.segment_id,
    ...
    const { data: row, error } = await client.from("holiday_entries").insert({
  ...
  if (method === "DELETE") {
    ...
    const { data: entry, error: e1 } = await client.from("holiday_entries").select("id, calendar_id").eq("id", id).single();
    if (e1 || !entry) {
      res.status(404).json({ error: "not found" });
      return;
    }
    const { data: cal } = await client.from("holiday_calendars").select("market_id, operating_model_id, segment_id").eq("id", entry.calendar_id).single();
    if (!cal) {
      res.status(404).json({ error: "calendar not found" });
      return;
    }
