...
// api/_revisionsImpl.ts
var revisionsImpl_exports = {};
__export(revisionsImpl_exports, {
  default: () => handler
});
module.exports = __toCommonJS(revisionsImpl_exports);
...
async function getRevisionHistory(fragmentType, fragmentId, limit = 50) {
  const client = supabaseServiceClient();
  const { data, error } = await client.from("config_revisions").select("id, version_number, created_at, created_by, change_summary").eq("fragment_type", fragmentType).eq("fragment_id", fragmentId).order("version_number", { ascending: false }).limit(limit);
  if (error) {
...
async function getRevisionSnapshot(revisionId) {
  const client = supabaseServiceClient();
  const { data, error } = await client.from("config_revisions").select("snapshot").eq("id", revisionId).single();
...
async function handler(req, res) {
  ...
  const history = await getRevisionHistory(table, id);
