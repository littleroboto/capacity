...
// api/lib/env.ts
function loadRepoDotEnvOnce() {
  if (_repoDotenvLoaded) return;
  _repoDotenvLoaded = true;
  const root = process.cwd();
  for (const name of [".env", ".env.local"]) {
    const full = import_node_path.default.join(root, name);
    if (!import_node_fs.default.existsSync(full)) continue;
    let text = import_node_fs.default.readFileSync(full, "utf8");
...
function requiredWithFallback(primary, fallback) {
...
function supabaseUrlFromPostgresEnv() {
  const keys = ["POSTGRES_URL", "POSTGRES_PRISMA_URL", "DATABASE_URL"];
...
function tryDeriveSupabaseRestUrl(postgresDsn) {
  if (!/supabase\.(co|com)/i.test(postgresDsn)) return void 0;
...
function supabaseUrlMissingHint(_hasPostgresUrl) {
  ...
  if (vercelEnv === "development" || process.env.VERCEL === "1" && !vercelEnv) {
    return " `vercel dev` uses the Development environment by default, not Production. Run `pnpm vercel:env:pull-prod` to write Production values into `.env.local`, or duplicate `SUPABASE_URL` (and service key) under Development in the Vercel dashboard.";
  }
  return "";
}
function serverEnv() {
  ...
  _cached = {
    ...
    supabaseUrl: (() => {
      const direct = optionalFirst(
        ["SUPABASE_URL", "NEXT_PUBLIC_SUPABASE_URL", "VITE_PUBLIC_SUPABASE_URL"]
      );
      if (direct) return direct;
      const derived = supabaseUrlFromPostgresEnv();
      if (derived) return derived;
      const hasPg = optionalFirst(["POSTGRES_URL", "POSTGRES_PRISMA_URL", "DATABASE_URL"]);
      throw new Error(
        `Missing required env var: one of SUPABASE_URL, NEXT_PUBLIC_SUPABASE_URL, VITE_PUBLIC_SUPABASE_URL` + (hasPg ? " (a Postgres URL is set but it is not a Supabase-hosted DSN we can map to a REST URL; set SUPABASE_URL explicitly.)" : " \u2014 Vercel serverless only sees variables enabled for this deployment environment (Preview vs Production vs Development). `.env.local` is not deployed unless mirrored in the Vercel project or via `vercel env pull` / dashboard.") + supabaseUrlMissingHint(Boolean(hasPg))
      );
    })(),
    supabaseServiceRoleKey: requiredFirst(["SUPABASE_SERVICE_ROLE_KEY", "SUPABASE_SECRET_KEY"]),
    supabaseAnonKey: optional("VITE_PUBLIC_SUPABASE_ANON_KEY") ?? optional("NEXT_PUBLIC_SUPABASE_ANON_KEY"),
    postgresUrl: optionalFirst(["POSTGRES_URL", "POSTGRES_PRISMA_URL", "DATABASE_URL"]),
    ...
    orgAdminRoles: optionalList("CAPACITY_ORG_ADMIN_ROLES").length ? optionalList("CAPACITY_ORG_ADMIN_ROLES") : ["admin"],
    clerkAuthorizedParties: optionalList("CAPACITY_CLERK_AUTHORIZED_PARTIES"),
    configSource: configSource === "postgres" ? "postgres" : "blob"
  };
  return _cached;
}
var import_node_fs, import_node_path, _repoDotenvLoaded, _cached;
var init_env = __esm({
  "api/lib/env.ts"() {
    "use strict";
...
// api/_buildsImpl.ts
var buildsImpl_exports = {};
__export(buildsImpl_exports, {
  default: () => handler
});
module.exports = __toCommonJS(buildsImpl_exports);

// api/lib/scopeResolver.ts
init_supabaseClient();
async function resolveUserScope(clerkUserId, email) {
  const client = supabaseServiceClient();
  const { data: scopes, error } = await client.from("user_access_scopes").select("*").eq("clerk_user_id", clerkUserId).eq("is_active", true);
  if (error) {
  ...
  if (!scopes || scopes.length === 0) {
    ...
    if (em) {
      ...
      for (const cand of candidates) {
        const { data, error: error2 } = await client.from("user_access_scopes").select("*").eq("is_active", true).eq("email", cand);
        if (error2) {
          console.error("[scopeResolver] email fallback query failed:", error2);
          break;
        }
        if (data && data.length > 0) {
          return mergeScopes(clerkUserId, email, data);
        }
      }
    }
    return emptyScope(clerkUserId, email);
  }
  return mergeScopes(clerkUserId, email, scopes);
}
function scopeRowIds(row) {
  const s = row;
  const om = s.operating_model_id ?? s.operatingModelId;
  const seg = s.segment_id ?? s.segmentId;
  const mkt = s.market_id ?? s.marketId;
  const role = s.role ?? void 0;
  return {
    operatingModelId: om,
    segmentId: seg,
    marketId: mkt,
...
function emptyScope(userId, email) {
  return {
    userId,
    email,
    isAdmin: false,
...
function mergeScopes(userId, email, scopes) {
  ...
  const opModelIds = /* @__PURE__ */ new Set();
...
function scopeAllowsMarket(scope, marketId, segmentId, operatingModelId) {
  if (scope.isAdmin) return true;
  if (scope.marketIds.includes(marketId)) return true;
  if (scope.segmentIds.includes(segmentId)) return true;
  if (scope.operatingModelIds.includes(operatingModelId) && scope.segmentIds.length === 0 && scope.marketIds.length === 0) {
    return true;
  }
  return false;
}
function scopeAllowsMarketEdit(scope, marketId, segmentId, operatingModelId) {
...
init_env();
async function verifyClerkBearerToken(bearer) {
  const env = serverEnv();
  const base = { secretKey: env.clerkSecretKey };
  const parties = env.clerkAuthorizedParties;
  if (parties.length > 0) {
    try {
      return await verifyToken2(bearer, {
        ...base,
        authorizedParties: parties
      });
    } catch (e) {
      try {
        const out = await verifyToken2(bearer, base);
  ...
  return await verifyToken2(bearer, base);
}

// api/lib/authScope.ts
function isServerConfigurationError(e) {
  if (!(e instanceof Error)) return false;
  return e.message.includes("Missing required env var") || e.message.includes("Missing required env var: one of");
}
function jwtEmailFromPayload(payload) {
  const pick = (v) => typeof v === "string" && v.includes("@") ? v.trim() : void 0;
...
// api/services/assemblyPipeline.ts
var import_crypto2 = require("crypto");
init_supabaseClient();
async function loadMarketFragments(marketId) {
  const client = supabaseServiceClient();
  const { data: market } = await client.from("markets").select("*").eq("id", marketId).eq("is_active", true).single();
  ...
  return {
    market,
    marketConfig: marketConfigRes.data ?? void 0,
    resourceConfig: resourceRes.data ?? void 0,
    bauConfig: bauRes.data ?? void 0,
    campaigns: campaignRes.data ?? [],
    techProgrammes: techProgRes.data ?? [],
    publicHolidayCalendar: publicHolRes.data ?? void 0,
    schoolHolidayCalendar: schoolHolRes.data ?? void 0,
    nationalLeaveBands: leaveBandRes.data ?? [],
    tradingConfig: tradingRes.data ?? void 0,
    deploymentRiskConfig: riskRes.data ?? void 0,
    operatingWindows: windowRes.data ?? []
  };
}
function holidayCalendarEntryDates(cal) {
...
function calBool(cal, camel, snake) {
  const v = cal[camel] ?? cal[snake];
  return Boolean(v);
}
function calNum(cal, camel, snake, fallback) {
  const v = cal[camel] ?? cal[snake];
  const n = Number(v);
  return Number.isFinite(n) ? n : fallback;
}
function assembleMarketYamlObject(fragments) {
  const yaml = {};
  ...
  if (fragments.resourceConfig) {
    ...
    if (rc.staffCapacity != null) {
      const staff = { capacity: rc.staffCapacity };
  ...
  if (fragments.publicHolidayCalendar) {
    const cal = fragments.publicHolidayCalendar;
    const pub = {
      auto: calBool(cal, "autoImport", "auto_import"),
      dates: holidayCalendarEntryDates(cal),
      staffing_multiplier: calNum(cal, "staffingMultiplier", "staffing_multiplier", 1),
      trading_multiplier: calNum(cal, "tradingMultiplier", "trading_multiplier", 1)
    };
    const loadEffects = cal.loadEffects ?? cal.load_effects;
    if (loadEffects && typeof loadEffects === "object" && !Array.isArray(loadEffects) && Object.keys(loadEffects).length > 0) {
      pub.load_effects = loadEffects;
    }
    yaml.public_holidays = pub;
  }
  if (fragments.schoolHolidayCalendar) {
    const cal = fragments.schoolHolidayCalendar;
    const school = {
      auto: calBool(cal, "autoImport", "auto_import"),
      dates: holidayCalendarEntryDates(cal),
      staffing_multiplier: calNum(cal, "staffingMultiplier", "staffing_multiplier", 1),
  ...
  if (fragments.marketConfig?.stressCorrelations && Object.keys(fragments.marketConfig.stressCorrelations).length > 0) {
    yaml.stress_correlations = fragments.marketConfig.stressCorrelations;
  }
  return yaml;
}
function toYamlText(obj) {
  const lines = [];
...
function marketRowHierarchy(market) {
  ...
  return { operatingModelId: om, segmentId: seg };
}
async function buildMarket(marketId, actorId) {
  const client = supabaseServiceClient();
  const fragments = await loadMarketFragments(marketId);
  if (!fragments) {
    return { build: { status: "failed" }, error: `Market ${marketId} not found` };
  }
  const hier = marketRowHierarchy(fragments.market);
  const { data: buildRow, error: buildErr } = await client.from("config_builds").insert({
  ...
  if (buildErr || !buildRow) {
    return { build: { status: "failed" }, error: buildErr?.message };
  }
  ...
  try {
    ...
    await client.from("config_builds").update({ status: "generated", completed_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", build.id);
    const { data: artifactRow } = await client.from("config_artifacts").insert({
      build_id: build.id,
      operating_model_id: hier.operatingModelId,
      segment_id: hier.segmentId,
      market_id: marketId,
    ...
    await client.from("config_builds").update({ status: "validated" }).eq("id", build.id);
    ...
    return {
      build: { ...build, status: "validated" },
      artifact: artifactRow
    };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await client.from("config_builds").update({ status: "failed", error_message: msg, completed_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", build.id);
    await client.from("audit_events").insert({
    ...
    return { build: { ...build, status: "failed" }, error: msg };
  }
}
async function publishBuild(buildId, actorId) {
  const client = supabaseServiceClient();
  ...
  if (build.market_id) {
    const { data: current } = await client.from("config_artifacts").select("id").eq("market_id", build.market_id).eq("artifact_type", "market_yaml").not("published_at", "is", null).is("superseded_at", null).single();
    if (current) {
      await client.from("config_artifacts").update({ superseded_at: now, superseded_by: artifact.id }).eq("id", current.id);
      await client.from("audit_events").insert({
  ...
  await client.from("config_artifacts").update({ published_at: now, published_by: actorId }).eq("id", artifact.id);
  await client.from("config_builds").update({ status: "published" }).eq("id", buildId);
  await client.from("audit_events").insert({
...
async function recordBuildComponents(buildId, fragments) {
  const client = supabaseServiceClient();
  const components = [];
  ...
  for (const { type, fragment } of fragmentSources) {
    ...
    const rev = await getLatestRevisionId(type, fragment.id);
    if (rev) {
      components.push({
        build_id: buildId,
        fragment_type: type,
  ...
  const arrayFragments = [
    { type: "campaign_configs", items: fragments.campaigns },
    { type: "tech_programme_configs", items: fragments.techProgrammes },
    { type: "national_leave_band_configs", items: fragments.nationalLeaveBands },
    { type: "operating_window_configs", items: fragments.operatingWindows }
  ];
  for (const { type, items } of arrayFragments) {
    for (const item of items) {
      const rev = await getLatestRevisionId(type, item.id);
      if (rev) {
        components.push({
          build_id: buildId,
          fragment_type: type,
...
async function getLatestRevisionId(fragmentType, fragmentId) {
  const client = supabaseServiceClient();
  const { data } = await client.from("config_revisions").select("id, version_number").eq("fragment_type", fragmentType).eq("fragment_id", fragmentId).order("version_number", { ascending: false }).limit(1).single();
  if (!data) return null;
  return { id: data.id, versionNumber: data.version_number };
}
var YAML_SECTION_ORDER = [
  "market",
  "title",
  "description",
  "resources",
...
function serializeYamlObject(obj, lines, indent) {
  ...
  for (const key of keys) {
    ...
    if (typeof val === "string" || typeof val === "number" || typeof val === "boolean") {
      const formatted = typeof val === "string" && /[:\-#{}\[\],&*?|>!%@`]/.test(val) ? `'${val.replace(/'/g, "''")}'` : String(val);
          ...
          if (itemKeys.length > 0) {
            ...
            for (let i = 1; i < itemKeys.length; i++) {
              ...
              if (typeof v === "object" && !Array.isArray(v)) {
                lines.push(`${pad}    ${k}:`);
                serializeYamlObject(v, lines, indent + 3);
              } else {
      ...
      lines.push(`${pad}${key}:`);
      serializeYamlObject(val, lines, indent + 1);
    }
  }
}
...
var CacheKeys = {
  activeArtifact: (marketId) => `config:mkt:${marketId}:active`,
  buildArtifact: (buildId) => `config:build:${buildId}:artifact`,
  buildMeta: (buildId) => `config:build:${buildId}:meta`,
  marketSummary: (operatingModelId, marketId) => `readmodel:om:${operatingModelId}:mkt:${marketId}:summary`,
  segmentActive: (operatingModelId, segmentId) => `config:om:${operatingModelId}:seg:${segmentId}:active`,
  operatingModelActive: (operatingModelId) => `config:om:${operatingModelId}:active`
};
...
async function handler(req, res) {
  ...
  if (method === "POST") {
    ...
    if (action === "build") {
      ...
      if (!scopeAllowsMarketEdit(auth.scope, marketId, mkt.segment_id, mkt.operating_model_id)) {
    ...
    if (action === "publish") {
      ...
      if (build.market_id) {
        ...
        if (mkt && !scopeAllowsMarketEdit(auth.scope, build.market_id, mkt.segment_id, mkt.operating_model_id)) {
          res.status(403).json({ error: "forbidden" });
          return;
        }
      }
