...
${cause.stack}`;
    ...
    PostgrestQueryBuilder = class {
      ...
      * ```
      */
      constructor(url, { headers = {}, schema: schema2, fetch: fetch$1, urlLengthLimit = 8e3, retry: retry2 }) {
        this.url = url;
    ...
    PostgrestClient = class PostgrestClient2 {
      ...
      * ```
      */
      constructor(url, { headers = {}, schema: schema2, fetch: fetch$1, timeout, urlLengthLimit = 8e3, retry: retry2 } = {}) {
        this.url = url;
        this.headers = new Headers(headers);
  ...
  "node_modules/.pnpm/@supabase+realtime-js@2.103.2/node_modules/@supabase/realtime-js/dist/main/lib/transformers.js"(exports2) {
    ...
    var toArray2 = (value, type2) => {
      ...
      if (openBrace === "{" && closeBrace === "}") {
        ...
        return arr.map((val) => (0, exports2.convertCell)(type2, val));
      }
      return value;
    };
  ...
  "node_modules/.pnpm/@supabase+realtime-js@2.103.2/node_modules/@supabase/realtime-js/dist/main/RealtimeChannel.js"(exports2) {
    ...
    var RealtimeChannel = class _RealtimeChannel {
      ...
      on(type2, filter, callback) {
        ...
        const typeCheck = type2 === REALTIME_LISTEN_TYPES.PRESENCE || type2 === REALTIME_LISTEN_TYPES.POSTGRES_CHANGES;
        if (stateCheck && typeCheck) {
          this.socket.log("channel", `cannot add \`${type2}\` callbacks for ${this.topic} after \`subscribe()\`.`);
          throw new Error(`cannot add \`${type2}\` callbacks for ${this.topic} after \`subscribe()\`.`);
        }
      ...
      _on(type2, filter, callback) {
        ...
        const ref = this.channelAdapter.on(type2, callback);
        const binding = {
          type: typeLower,
          filter,
          callback,
...
// api/services/yamlImportService.ts
var yamlImportService_exports = {};
__export(yamlImportService_exports, {
  importMarketYamlObject: () => importMarketYamlObject
});
async function importMarketYamlObject(yamlObj, operatingModelId, segmentId, actorId, actorEmail) {
  ...
  if (Array.isArray(yamlObj.campaigns)) {
    for (const c of yamlObj.campaigns) {
      try {
        await client.from("campaign_configs").insert({
          ...
          promo_weight: c.promo_weight ?? c.business_uplift ?? 1,
  ...
  if (Array.isArray(yamlObj.tech_programmes)) {
    for (const p of yamlObj.tech_programmes) {
      try {
  ...
  if (Array.isArray(yamlObj.national_leave_bands)) {
    for (const b of yamlObj.national_leave_bands) {
      try {
  ...
  if (yamlObj.public_holidays) {
    try {
      const ph = yamlObj.public_holidays;
      const { data: cal } = await client.from("holiday_calendars").insert({
        ...meta,
        calendar_type: "public",
        auto_import: ph.auto ?? false,
        staffing_multiplier: ph.staffing_multiplier ?? 1,
        trading_multiplier: ph.trading_multiplier ?? 1,
        load_effects: null,
        extra_settings: {}
      }).select().single();
      if (cal && Array.isArray(ph.dates)) {
        const entries = ph.dates.map((d) => ({
          calendar_id: cal.id,
          holiday_date: d
        }));
        if (entries.length > 0) {
  ...
  if (yamlObj.school_holidays) {
    try {
      const sh = yamlObj.school_holidays;
      const { data: cal } = await client.from("holiday_calendars").insert({
        ...meta,
        calendar_type: "school",
        auto_import: sh.auto ?? false,
        staffing_multiplier: sh.staffing_multiplier ?? 1,
        trading_multiplier: sh.trading_multiplier ?? 1,
        load_effects: sh.load_effects ?? null,
        extra_settings: {}
      }).select().single();
      if (cal && Array.isArray(sh.dates)) {
        const entries = sh.dates.map((d) => ({
          calendar_id: cal.id,
          holiday_date: d
        }));
        if (entries.length > 0) {
  ...
  if (yamlObj.trading) {
    try {
      ...
      await client.from("trading_configs").insert({
        ...
        campaign_effect_scale: t.campaign_effect_scale ?? 1,
  ...
  if (Array.isArray(yamlObj.operating_windows)) {
    for (const w of yamlObj.operating_windows) {
      try {
...
function extractHolidaySettings(yamlObj) {
  const holidays = yamlObj.holidays;
  if (!holidays) return {};
  return {
    capacity_taper_days: holidays.capacity_taper_days ?? 0,
    lab_capacity_scale: holidays.lab_capacity_scale ?? 1
  };
}
function errorMsg(e) {
  return e instanceof Error ? e.message : String(e);
}
var init_yamlImportService = __esm({
  "api/services/yamlImportService.ts"() {
    "use strict";
    init_supabaseClient();
  }
...
// api/_importImpl.ts
var importImpl_exports = {};
__export(importImpl_exports, {
  default: () => handler
});
module.exports = __toCommonJS(importImpl_exports);
...
async function handler(req, res) {
  ...
  const client = supabaseServiceClient();
  const { data: mkt } = await client.from("markets").select("id, segment_id, operating_model_id").eq("id", marketId).single();
  if (!mkt) {
    res.status(404).json({ error: "market_not_found" });
    return;
  }
  if (!scopeAllowsMarketEdit(scope, mkt.id, mkt.segment_id, mkt.operating_model_id)) {
  ...
  let yaml;
  try {
    yaml = await Promise.resolve().then(() => (init_js_yaml(), js_yaml_exports));
  } catch {
  ...
  const { importMarketYamlObject: importMarketYamlObject2 } = await Promise.resolve().then(() => (init_yamlImportService(), yamlImportService_exports));
  const result = await importMarketYamlObject2(
    { ...parsed, market: marketId },
    mkt.operating_model_id,
    mkt.segment_id,
    scope.userId,
    scope.email
  );
...
function summarizeSections(obj) {
  const sections = [];
