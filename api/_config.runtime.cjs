...
async function withUserScope(scope, fn) {
  const client = supabaseServiceClient();
  const role = scope.isAdmin ? "admin" : scope.segmentIds.length > 0 && scope.marketIds.length === 0 ? "segment_editor" : scope.marketIds.length > 0 ? "market_editor" : "viewer";
...
// api/_configImpl.ts
var configImpl_exports = {};
__export(configImpl_exports, {
  default: () => handler
});
module.exports = __toCommonJS(configImpl_exports);
...
async function getActiveArtifact(marketId) {
  const cached = await cacheGet(CacheKeys.activeArtifact(marketId));
  if (cached) return cached;
  const { supabaseServiceClient: supabaseServiceClient2 } = await Promise.resolve().then(() => (init_supabaseClient(), supabaseClient_exports));
  const client = supabaseServiceClient2();
  const { data: published } = await client.from("config_artifacts").select("content").eq("market_id", marketId).eq("artifact_type", "market_yaml").not("published_at", "is", null).is("superseded_at", null).order("published_at", { ascending: false }).limit(1).maybeSingle();
  if (published && typeof published.content === "string") {
    const content = published.content;
    await cacheSet(CacheKeys.activeArtifact(marketId), content);
    return content;
  }
  const { data: validatedBuild } = await client.from("config_builds").select("id").eq("market_id", marketId).eq("status", "validated").order("created_at", { ascending: false }).limit(1).maybeSingle();
  if (validatedBuild?.id) {
    const { data: draftArt } = await client.from("config_artifacts").select("content").eq("build_id", validatedBuild.id).eq("artifact_type", "market_yaml").maybeSingle();
    if (draftArt && typeof draftArt.content === "string") {
      return draftArt.content;
    }
  }
  return null;
}
async function getMultiMarketBundle(marketIds) {
  const parts = [];
...
async function previewAssembledYamlForMarket(marketId) {
...
async function handler(req, res) {
  ...
  if (marketId) {
    ...
    if (mkt && !scopeAllowsMarket(scope, marketId, mkt.segment_id, mkt.operating_model_id)) {
    ...
    if (assembled) {
      const previewNotice = "# Assembled from live database fragments (preview \u2014 not a published artifact).\n# Use Build & Publish in admin to create a versioned published artifact.\n\n";
      res.setHeader("Content-Type", "text/yaml; charset=utf-8");
      res.setHeader("Cache-Control", "no-store");
    ...
    res.status(404).json({
      error: "No YAML for this market: no published or draft build artifact, and live assembly failed or the market is missing/inactive."
    });
    return;
  }
  if (markets) {
    ...
    for (const id of ids) {
      ...
      if (mkt && scopeAllowsMarket(scope, id, mkt.segment_id, mkt.operating_model_id)) {
        allowed.push(id);
      }
    }
