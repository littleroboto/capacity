...
// api/_marketsImpl.ts
var marketsImpl_exports = {};
__export(marketsImpl_exports, {
  default: () => handler
});
module.exports = __toCommonJS(marketsImpl_exports);
...
function sendJsonIfOpen(res, status, body) {
  if (res.headersSent) return;
  res.status(status).json(body);
}
async function enrichOneMarket(client, m) {
  const mid = String(m.id ?? "");
  const { data: latestBuild, error: buildErr } = await client.from("config_builds").select("status, created_at").eq("market_id", mid).order("created_at", { ascending: false }).limit(1).maybeSingle();
  if (buildErr) {
    console.error("[markets] config_builds for", mid, buildErr);
  }
  const { count: issueCount, error: valErr } = await client.from("validation_results").select("*", { count: "exact", head: true }).eq("target_id", mid).eq("severity", "error");
  if (valErr) {
...
async function handler(req, res) {
  ...
  const rid = requestId ? { requestId } : void 0;
  ...
  try {
    ...
    const client = supabaseServiceClient();
    step = "markets_query";
    const { data: rawMarkets, error: marketsError } = await client.from("markets").select("*, segments(label)").eq("is_active", true).order("display_order");
    if (marketsError) {
      console.error(
        JSON.stringify({
          tag: "markets",
    ...
    step = "normalize_rows";
    const markets = Array.isArray(rawMarkets) ? rawMarkets : rawMarkets && typeof rawMarkets === "object" ? [rawMarkets] : [];
    if (markets.length === 0) {
      res.status(200).json([]);
      return;
    }
    step = "filter";
    const filtered = scope.isAdmin ? markets : markets.filter(
      (row) => scopeAllowsMarket(
        scope,
        String(row.id ?? ""),
        String(row.segment_id ?? ""),
        String(row.operating_model_id ?? "")
      )
    );
