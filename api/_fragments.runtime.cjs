...
// api/_fragmentsImpl.ts
var fragmentsImpl_exports = {};
__export(fragmentsImpl_exports, {
  default: () => handler
});
module.exports = __toCommonJS(fragmentsImpl_exports);
...
// api/services/fragmentService.ts
async function getFragment(table, id) {
  const client = supabaseServiceClient();
...
async function listFragments(table, marketId, status) {
  const client = supabaseServiceClient();
  if (table === "holiday_calendars") {
    let query2 = client.from("holiday_calendars").select("*, holiday_entries(*)").eq("market_id", marketId);
    if (status) {
      query2 = query2.eq("status", status);
    }
    const { data: data2, error: error2 } = await query2.order("created_at", { ascending: false });
    if (error2) {
      throw new Error(`Failed to list ${table} for ${marketId}: ${error2.message}`);
    }
    return data2 ?? [];
  }
...
async function createFragment(table, data, actorId, actorEmail) {
  const client = supabaseServiceClient();
  const row = {
    ...data,
    version_number: 1,
...
async function updateFragment(table, id, updates, expectedVersion, actorId, actorEmail) {
  const client = supabaseServiceClient();
  const current = await getFragment(table, id);
  if (!current) {
  ...
  const { data: updated, error } = await client.from(table).update({
    ...patch,
    version_number: newVersion,
    updated_by: actorId
  }).eq("id", id).eq("version_number", expectedVersion).select().single();
  if (error) {
...
async function archiveFragment(table, id, expectedVersion, actorId, actorEmail) {
...
function rowVersionNumber(row) {
  const raw = row.versionNumber ?? row.version_number;
  const n = Number(raw);
  return Number.isFinite(n) ? n : NaN;
}
async function createRevision(fragmentType, fragmentId, versionNumber, snapshot, actorId) {
  const client = supabaseServiceClient();
  const opModelId = snapshot.operating_model_id ?? "";
  const segId = snapshot.segment_id ?? "";
  const mktId = snapshot.market_id ?? null;
...
async function logAudit(eventType, actorId, actorEmail, fragment, targetType) {
  const client = supabaseServiceClient();
...
async function lookupMarketMeta(marketId) {
  const client = supabaseServiceClient();
...
async function handler(req, res) {
  ...
  if (method === "GET") {
    ...
    if (id) {
      const frag = await getFragment(table, id);
      if (!frag) {
    ...
    if (marketId) {
      ...
      if (mkt && !scopeAllowsMarket(auth.scope, marketId, mkt.segment_id, mkt.operating_model_id)) {
        res.status(403).json({ error: "forbidden" });
        return;
      }
      const status = req.query.status;
      const data = await listFragments(table, marketId, status);
  ...
  if (method === "POST") {
    ...
    if (!marketId) {
      res.status(400).json({ error: "market_id required in body" });
      return;
    }
    const mkt = await lookupMarketMeta(marketId);
    if (!mkt) {
    ...
    const result = await createFragment(table, body, auth.scope.userId, auth.email);
    if (!result.ok) {
  ...
  if (method === "PUT") {
    ...
    if (expectedVersion == null) {
      res.status(400).json({ error: "expectedVersion required in body" });
      return;
    }
    const existing = await getFragment(table, id);
    if (!existing) {
      res.status(404).json({ error: "not found" });
      return;
    }
    ...
    const result = await updateFragment(table, id, body, Number(expectedVersion), auth.scope.userId, auth.email);
    if (!result.ok) {
  ...
  if (method === "DELETE") {
    ...
    if (!expectedVersion) {
      res.status(400).json({ error: "expectedVersion query param required" });
      return;
    }
    const existing = await getFragment(table, id);
    if (!existing) {
    ...
    const result = await archiveFragment(table, id, expectedVersion, auth.scope.userId, auth.email);
    if (!result.ok) {
