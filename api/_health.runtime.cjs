...
// api/_healthImpl.ts
var healthImpl_exports = {};
__export(healthImpl_exports, {
  default: () => handler
});
module.exports = __toCommonJS(healthImpl_exports);
...
async function handler(req, res) {
  ...
  try {
    ...
    const { count, error } = await client.from("markets").select("*", { count: "exact", head: true });
    if (error) {
