...
// api/_validateImpl.ts
var validateImpl_exports = {};
__export(validateImpl_exports, {
  default: () => handler
});
module.exports = __toCommonJS(validateImpl_exports);
...
// api/services/validationService.ts
...
function validateTechProgramme(programme) {
  const issues = [];
...
function validateTrading(config) {
  const issues = [];
...
function validateNationalLeaveBand(band) {
  const issues = [];
  if (!band.fromDate) {
...
function validateCrossFragment(fragments) {
  const issues = [];
...
async function persistValidationResults(issues, scope, targetType, targetId, buildId) {
...
function validateMarketFragments(fragments) {
  const issues = [];
  if (fragments.resourceConfig) {
...
async function handler(req, res) {
  ...
  if (req.method === "GET") {
    const { data } = await client.from("validation_results").select("*").eq("target_id", marketId).order("created_at", { ascending: false }).limit(100);
