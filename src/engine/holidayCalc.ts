...
import {
  ...
  FR_PUBLIC_ENTRIES,
  IT_PUBLIC_ENTRIES,
  getStubPublicHolidayName,
  NA_PUBLIC_ENTRIES,
  PL_PUBLIC_ENTRIES,
  UK_PUBLIC_ENTRIES,
} from '@/engine/holidayPublicCatalog';
...
const AUTO_PUBLIC_BY_MARKET: Record<string, string[]> = {
  UK: UK_PUBLIC,
  DE: DE_PUBLIC,
  FR: FR_PUBLIC,
  AU: AU_PUBLIC,
...
const AUTO_SCHOOL_BY_MARKET: Record<string, string[]> = {
  AU: AU_SCHOOL,
  UK: UK_SCHOOL,
  DE: DE_SCHOOL,
  FR: FR_SCHOOL,
...
export function getAutoHolidayDates(
  ...
  const dates: string[] = [];
  if (options.auto_public && AUTO_PUBLIC_BY_MARKET[market]) {
    dates.push(...AUTO_PUBLIC_BY_MARKET[market]);
  }
  if (options.auto_school) {
    const school = AUTO_SCHOOL_BY_MARKET[market];
