...
/**
 * Character range in the **patched** string (before any post-normalize) covering the first patch’s `new`
 * text, with `start`/`end` adjusted when later patches fall earlier in the document (non-overlap assumed).
 */
export function patchFocusCharRangeInPatchedText(
  source: string,
  ...
  for (let i = 0; i < patches.length; i++) {
    ...
    const delta = p.new.length - p.old.length;
    if (i === 0) {
      start = idx;
      end = idx + p.new.length;
    } else if (start >= 0) {
      if (idx + p.old.length <= start) {
        start += delta;
        end += delta;
      } else if (idx < end) {
        end = Math.max(end, idx + p.new.length);
      }
    }
...
/**
 * Map a character range from the patched buffer to `normalizeAssistantYaml(patched)` when they differ.
 */
export function mapPatchFocusToNormalizedBuffer(
  patched: string,
  normalized: string,
  focus: { start: number; end: number }
): { start: number; end: number } | null {
  if (patched === normalized) {
    return { start: focus.start, end: Math.max(focus.end, Math.min(focus.start + 1, normalized.length)) };
  }
  const span = Math.max(1, focus.end - focus.start);
  const slice = patched.slice(focus.start, focus.start + Math.min(span, 2000));
  if (slice.length > 0) {
    const i = normalized.indexOf(slice);
    if (i !== -1) {
      return { start: i, end: Math.min(i + span, normalized.length) };
    }
  }
  const ctxBefore = patched.slice(Math.max(0, focus.start - 120), focus.start);
  if (ctxBefore.length > 0) {
    const i = normalized.indexOf(ctxBefore);
    if (i !== -1) {
      const at = i + ctxBefore.length;
      return { start: at, end: Math.min(at + 1, normalized.length) };
    }
  }
  const lineStart = focus.start <= 0 ? 0 : patched.lastIndexOf('\n', focus.start - 1) + 1;
  const lineEndIdx = patched.indexOf('\n', focus.start);
  const line =
    lineEndIdx === -1 ? patched.slice(lineStart) : patched.slice(lineStart, lineEndIdx);
  if (line.length > 0) {
    const j = normalized.indexOf(line);
    if (j !== -1) {
      return { start: j, end: Math.min(j + line.length, normalized.length) };
    }
  }
  return null;
}
