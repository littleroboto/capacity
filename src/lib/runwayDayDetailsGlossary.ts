...
export function glossaryFillScore(mode: ViewModeId): string {
  if (mode === 'combined') {
    return 'Raw tech-capacity fill behind this heatmap, before colour transfer and gamma. It is the maximum of lab, Market IT, and (half-weighted) backend load versus effective capacity—uncapped, so values above 1 mean over 100% demand on the tightest lane. The large percentage in the tile is this value × 100%.';
  }
