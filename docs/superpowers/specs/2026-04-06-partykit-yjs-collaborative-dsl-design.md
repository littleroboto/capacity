...
- **Align with existing ACL**: session claims **`cap_admin`**, **`cap_segs`**, **`cap_mkts`**, **`cap_ed`** (see [AUTH_PROVIDER.md](../AUTH_PROVIDER.md)) and server behaviour in [`api/_sharedDslImpl.ts`](../../api/_sharedDslImpl.ts) (GET **filter**, PUT **merge** of allowed market documents).
...
1. Verifies **Clerk JWT** (same trust model as [`api/_sharedDslImpl.ts`](../../api/_sharedDslImpl.ts) + [`api/_allowedUserEmails.ts`](../../api/_allowedUserEmails.ts)).
...
- [`api/_sharedDslImpl.ts`](../../api/_sharedDslImpl.ts) — filter + partial merge (ACL inlined; bundled for Vercel)
