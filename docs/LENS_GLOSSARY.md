...
| `market_risk` | Deployment Risk | Deployment Risk | **Deployment / calendar fragility** (0–1): holidays, Q4/month curve, trading intensity, campaigns × peaks, tech bench strain, YAML events. Hotter = more fragile, not a ban. | Not the same construct as the Technology headroom tile. | Blend is the full operational mix; band still reflects that mix while the tile shows deployment-risk score only. | `deployment_risk_01`, `heatmapCellMetric` → `market_risk` |
